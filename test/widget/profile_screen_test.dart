...
void main() {
  ...
  const testChildren = <ChildModel>[
    ChildModel(id: 101, fullName: 'Eshmatov Ali', className: '1A', classId: 10),
    ChildModel(
      id: 102,
  ...
  setUp(() {
    mockUserNotifier = MockUserNotifier(
      const UserState(user: testUser, children: testChildren),
    );

    mockPaymentNotifier = MockPaymentNotifier(
      const PaymentState(balance: testBalance),
    );

    mockAuthNotifier = MockAuthNotifier(const AuthState(isAuthenticated: true));
  ...
  group('ProfileScreen Widget Tests', () {
    testWidgets('renders user information and metrics properly', (
      tester,
    ) async {
    ...
    testWidgets(
      'taps on Settings items redirects or shows snackbar appropriately',
      (tester) async {
